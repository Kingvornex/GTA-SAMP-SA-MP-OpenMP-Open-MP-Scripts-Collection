/*=========================================================================================|
|----------------------------Created By: Josh Greening---------------------|
|-------------------------------------Siren Filterscript-----------------------------------|
|==========================================================================================*/
/*|---------------------------------Don't remove Credits from script------------------------|
===========================================================================================*/
#include <a_samp>
#include <zcmd>
...
CMD:policelights(playerid, params[]) return cmd_siren(playerid, params);
CMD:siren(playerid, params[])
{
        ...
        if(!Flasher[vehicleid]) {
                ...
                {
                        return SendClientMessage(playerid, -1, "* You are not sitting near the switch for the siren!");
                }
                if (!GetVehicleModel(vehicleid)) return SendClientMessage(playerid, -1, "* You are not in an LEO vehicle!");
                ...
                {
                        SendClientMessage(playerid, -1, "You have turned off the lights."), DestroyObject(obj[vehicleid]), DestroyObject(obj2[vehicleid]), DestroyObject(obj3[vehicleid]), DestroyObject(obj4[vehicleid]), DestroyObject(obj5[vehicleid]);
                ...
                switch (GetVehicleModel(vehicleid))
                {
                        ...
                        default:
                        {
                                return SendClientMessage(playerid, -1, "You are not in a law enforcement agencies vehicle!");
                        }
                }
                return SendClientMessage(playerid, -1, "You have turned on the strobe lights.");
        } else {
                if (IsValidObject(obj[vehicleid]) || IsValidObject(obj2[vehicleid]) || IsValidObject(obj3[vehicleid]) || IsValidObject(obj4[vehicleid]) || IsValidObject(obj5[vehicleid])) {
                        SendClientMessage(playerid, -1, "You switched off the strobe lights."), DestroyObject(obj[vehicleid]), DestroyObject(obj2[vehicleid]), DestroyObject(obj3[vehicleid]), DestroyObject(obj4[vehicleid]), DestroyObject(obj5[vehicleid]);
                }
                GetVehicleDamageStatus(vehicleid,panels,doors,lights,tires);
                if(LightPwr[vehicleid] == 1)
                        UpdateVehicleDamageStatus(vehicleid, panels, doors, 0, tires);
...
public OnFilterScriptInit()
{
        ...
        FlashTimer = SetTimer("FlasherFunc",300,1);
        return 1;
}
